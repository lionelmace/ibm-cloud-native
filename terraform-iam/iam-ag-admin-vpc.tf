...
resource "ibm_iam_access_group" "ag-admin-vpc" {
  name = format("%s-%s", local.basename, "ag-admin-vpc")
  tags = var.tags
}
...
resource "ibm_iam_access_group_policy" "policy_vpc" {
  access_group_id = ibm_iam_access_group.ag-admin-vpc.id
  roles           = ["Editor"]
  ...
  resources {
    ...
    # Uncomment to apply the policy to a specific Resource Group
