...
resource "ibm_cos_bucket" "logs-bucket-data" {
  bucket_name          = format("%s-%s-%s", local.basename, "bucket-data", random_string.random.0.result)
...
resource "ibm_cos_bucket" "logs-bucket-metrics" {
  bucket_name          = format("%s-%s-%s", local.basename, "bucket-metrics", random_string.random.0.result)
  resource_instance_id = ibm_resource_instance.cos-for-logs.id
  storage_class        = "smart"
